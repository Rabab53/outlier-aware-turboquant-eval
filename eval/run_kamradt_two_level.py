...
def plot_heatmap(data, context_lengths, depths, filename, title_base="Evaluation"):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_context", type=int, default=100000, help="Maximum context length")
    ...
    # We sweep across different Two-Level configurations
    configs = [
        {"inlier": 2, "outlier": 4, "out_frac": 0.05},
        {"inlier": 2, "outlier": 4, "out_frac": 0.10},
        {"inlier": 2, "outlier": 6, "out_frac": 0.05},
        {"inlier": 2, "outlier": 6, "out_frac": 0.10},
    ]

    for config in configs:
        inlier_bits = config["inlier"]
        outlier_bits = config["outlier"]
        out_frac = config["out_frac"]
        pct = int(out_frac * 100)
        
        name = f"kamradt_twolevel_in{inlier_bits}b_out{outlier_bits}b_{pct}out"
        ...
        for d in depths:
            ...
            for c in context_lengths:
                score = run_kamradt_multi_needle(model, tokenizer, head_dim, inlier_bits=inlier_bits, outlier_bits=outlier_bits, outlier_fraction=out_frac, context_length=c, depth=d, haystack_tokens=haystack_tokens)
                row.append(score)
        ...
        title = f"{short_name} Two-Level (In: {inlier_bits}b, Out: {outlier_bits}b, {pct}%)"
        plot_heatmap(matrix, context_lengths, depths, os.path.join(args.out_dir, f"{name}.png"), title_base=title)
